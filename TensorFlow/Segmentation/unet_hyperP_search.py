...
adapted for a cloud detection dataset.
        ...
        -Ouputs
                    ...
                    .h5 model
                    learning curves  (plot_loss_accuracy_graphs)
...
def get_optimizer(name, trial,learning_rate):
    if name == 'SGD':
...
def image_generator(files, batch_size, sz = Image_size):
...
def objective(trial):
    ...
    def build_callbacks():
            checkpointer = ModelCheckpoint(filepath='unet_hypSearch.h5', verbose=0, save_best_only=True)
    ...
    class PlotLearning(keras.callbacks.Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            subfolder = "Unet_results_HyperP_search/trial_"+str(trial.number)
            os.makedirs(subfolder, exist_ok=True)
            ...
            # Save the plots as images
            output_filename = os.path.join(subfolder, "loss_accuracy_"+str(trial.number)+".png")
            plt.savefig(output_filename, bbox_inches="tight")
            plt.close()
...
trials_df.to_csv('results_HyperP_search/optuna_results_unet.csv', index=False)

# # Save the DataFrame to an .xlsx file
trials_df.to_excel('results_HyperP_search/optuna_results_unet.xlsx', index=False)
print("Best Hyperparameters_unet:", best_params)

#plot = plot_optimization_history(study)
#plot.write_image("results_HyperP_search/optimization_history_unet.png")
#plot1 = plot_param_importances(study)
#plot1.write_image("results_HyperP_search/param_unet.png")
#plot2 = plot_slice(study)
#plot2.write_image("results_HyperP_search/slice_unet.png")
#plot3 = plot_contour(study)
#plot3.write_image("results_HyperP_search/contour_unet.png")
